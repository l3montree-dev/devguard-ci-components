...
export async function transformToGitHubCompatibleYaml(
  pipeline: GitLabCI,
  filePath: string,
): Promise<void> {
...
export async function ExportGitHubActionsWorkflows(
  pipeline: GitLabCI,
  filePath: string,
...
export async function ExportCIComponents(
  ...
  if (!Bun.file("./github/").exists()) await mkdir("./github/");
  ...
  for (const [templateName, template] of Object.entries(templates)) {
    ...
    const pipeline: GitLabCI = {
      ...
      jobs: template
        .filter((t): t is JobWithSpec => "job" in t)
        .map((def) => ({ [def.name]: def.job }))
        .reduce((acc, curr) => ({ ...acc, ...curr }), {}) as GitLabCI["jobs"],
      include: template
