...
export type StringifyOptions = Extract<
  NonNullable<Parameters<typeof stringify>[2]>,
  object
>;
...
async function transformToGitHubCompatibleYaml(pipeline: GitLabCI, filePath: string): Promise<void> {
  ...
  const yaml = stringifyData(githubPipeline);
  await writeFile(filePath, yaml);
}
async function ExportGitHubActionsWorkflows(pipeline: GitLabCI, filePath: string, header = "# This file is automatically generated. Do not edit directly.\n\n"): Promise<void> {
  ...
  const yaml = stringifyData(githubWorkflow);
  await writeFile(filePath, header + yaml);
 ...
 */
function checkForInputConflicts(templates: (GitHubWorkflow | GitLabJobWithSpec | IncludeWithSpec)[], ignoreKeys: string[] = []): void {
  ...
  for (const [templateName, template] of Object.entries(templates)) {
    ...
    if (githubJobs.length > 0) {
      ...
      const githubWorkflow: GitHubWorkflowReusable = {
        ...
        jobs: githubJobs.reduce((acc, def) => ({ ...acc, [templateSlug(def.name)]: def.job }), {}),
      };
