...
async function parseYamlFile(absPath: string, root: string): Promise<ParsedFile[]> {
  ...
  let docs: any[];
  try {
    docs = [YAML.parse(text)];
  } catch (e) {
    if ((e as any).code === "MULTIPLE_DOCS") {
      try {
        docs = YAML.parseAllDocuments(text).map((d: any) => d.toJSON());
      } catch (e2) {
        console.error(`Failed to parse multiple docs in ${relPath}:`, e2);
...
async function main() {
  ...
  const parsed = (await Promise.all(files.map((f) => parseYamlFile(f, root)))).flat();
